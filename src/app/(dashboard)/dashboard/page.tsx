...
export default function DashboardPage() {
  ...
  const totalSales     = invoices.filter(i => i.status === "INVOICE_PAID").reduce((s, i) => s + i.total, 0);
  const recentInvoices = [...invoices].sort((a, b) => new Date(b.createdAt).getTime() - new Date(a.createdAt).getTime()).slice(0, 10);
              ...
              : recentInvoices.map(inv => (
                <div key={inv.id} className="flex items-center justify-between py-2 border-b border-white/5 last:border-0">
                  <div>
                    ...
                    <p className="text-xs text-slate-500">{inv.customer.firstName} {inv.customer.lastName}</p>
                  </div>
        ...
        content: (
          <div className="p-4 space-y-1">
            {lowStockProducts.length === 0
              ? <p className="text-slate-500 text-sm text-center py-4">Todo el inventario en orden ✓</p>
        ...
        content: (
          <div className="p-4 space-y-1">
            {opportunities.length === 0
              ? <p className="text-slate-500 text-sm text-center py-4">Sin oportunidades registradas</p>
              : opportunities.slice(0, 15).map(o => (
                <div key={o.id} className="flex items-center justify-between py-2 border-b border-white/5 last:border-0">
                  <div>
                    ...
                    <p className="text-xs text-slate-500">{o.stage?.name ?? "—"}</p>
                  </div>
                  <div className="text-right">
                    {o.amount != null && <p className="text-sm font-bold text-white">${o.amount.toFixed(0)}</p>}
  ...
  }, [recentInvoices, lowStockProducts, opportunities, setTabs]);
    ...
    <div className="flex flex-col h-full">
      ...
      <div className="flex-1 p-6 space-y-5">
        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
          ...
          />
          <StatCard title="Clientes" value={customers} icon={Users} color="purple" />
          <StatCard
            title="Oportunidades"
            value={openOpps}
            icon={Activity}
        ...
        <p className="text-xs text-slate-600 px-1">
          Últimas facturas · Stock bajo · Oportunidades — disponibles en el panel inferior ↓
        </p>
      </div>
    </div>
  );
