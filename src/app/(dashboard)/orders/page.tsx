          ...
          <div key={step} className="flex gap-3">
            <div className="flex flex-col items-center">
              <div className={`w-7 h-7 rounded-full border-2 flex items-center justify-center shrink-0 transition-all ${
                current ? `${meta.dot.replace("bg-", "border-")} ${meta.dot} shadow-lg` : done ? "border-emerald-500 bg-emerald-500/20" : "border-white/10 bg-white/5"
              }`}>
            ...
            <div className="pb-4 flex-1 min-w-0">
              <div className="flex items-center gap-2 flex-wrap">
                <p className={`text-sm font-semibold ${done ? "text-white" : "text-slate-500"}`}>{meta.label}</p>
                {current && <span className="text-[10px] px-1.5 py-0.5 rounded-full bg-blue-500/20 text-blue-300 border border-blue-500/30 font-medium">ACTUAL</span>}
              </div>
              {event ? (
                <div className="mt-0.5">
                  {event.notes && <p className="text-xs text-slate-400 leading-relaxed">{event.notes}</p>}
                  <p className="text-[10px] text-slate-600 mt-0.5">{new Date(event.createdAt).toLocaleString("es")}{event.createdBy && ` · ${event.createdBy}`}</p>
                </div>
              ) : (
      ...
      {tracking.filter(t => ["CANCELLED","RETURNED","ORDER_CANCELLED","ORDER_RETURNED"].includes(t.status)).map(ev => (
        <div key={ev.id} className="flex gap-3 mt-1">
          ...
          <div className="pb-2 flex-1">
            <p className="text-sm font-semibold text-red-400">{STATUS_META[normalizeStatus(ev.status)]?.label ?? ev.status}</p>
            {ev.notes && <p className="text-xs text-slate-400 mt-0.5">{ev.notes}</p>}
            <p className="text-[10px] text-slate-600 mt-0.5">{new Date(ev.createdAt).toLocaleString("es")}{ev.createdBy && ` · ${ev.createdBy}`}</p>
          </div>
        </div>
      ))}
    </div>
...
export default function OrdersPage() {
  ...
  const load = async () => {
    ...
    if (r.success) {
      const all = r.data;
      setOrders(filterStatus === "ALL" ? all : all.filter(o => normalizeStatus(o.status) === filterStatus));
    }
    setLoading(false);
  };
  ...
  const createOrder = async () => {
    ...
    const body: CreateOrderRequest = {
      customerId: Number(form.customerId), warehouseId: Number(form.warehouseId),
      taxRate: form.taxRate, shippingAddress: form.shippingAddress || undefined,
      estimatedDelivery: form.estimatedDelivery || undefined, notes: form.notes || undefined,
  ...
  const saveStatus = async () => {
    ...
    if (r.success) { load(); if (detailOrder?.id === statusModal.id) openDetail(r.data); }
    setStatusModal(null);
  ...
  const columns: Column<Order>[] = [
    { key: "orderNumber", label: "Pedido", render: o => <span className="font-mono text-blue-400 text-sm">{o.orderNumber}</span> },
    { key: "customer", label: "Cliente", render: o => <span className="text-slate-200">{o.customer.firstName} {o.customer.lastName}</span> },
    { key: "status", label: "Estado", render: o => <StatusBadge status={o.status} /> },
    { key: "total", label: "Total", render: o => <span className="text-emerald-400 font-semibold">${o.total.toFixed(2)}</span> },
    { key: "trackingCode", label: "Guía", render: o => <span className="text-slate-500 text-xs font-mono">{o.trackingCode ?? "—"}</span> },
    { key: "estimatedDelivery", label: "Entrega est.", render: o => <span className="text-slate-400 text-xs">{o.estimatedDelivery ? new Date(o.estimatedDelivery).toLocaleDateString("es") : "—"}</span> },
    { key: "createdAt", label: "Creado", render: o => <span className="text-slate-500 text-xs">{new Date(o.createdAt).toLocaleDateString("es")}</span> },
    { key: "actions", label: "", render: o => (
      <div className="flex gap-1" onClick={e => e.stopPropagation()}>
        <button onClick={() => openDetail(o)} className="btn-secondary px-2 py-1.5 text-xs" title="Ver seguimiento"><Eye className="w-3 h-3" /></button>
        {NEXT_STATUSES[normalizeStatus(o.status)]?.length && (
          <button onClick={() => openStatusModal(o)} className="btn-primary px-2 py-1.5 text-xs" title="Actualizar estado"><Truck className="w-3 h-3" /></button>
        )}
      </div>
    )},
  ];
  ...
  return (
    <div className="flex flex-col h-full">
      ...
      <div className="flex-1 p-6 space-y-4">
        ...
        <p className="text-xs text-slate-600 px-1">El listado de pedidos está en el panel inferior ↓</p>
      </div>
            ...
            <div className="space-y-4">
              <div className="grid grid-cols-2 gap-2 text-sm">
                {([
                  ...
                  ["Entrega est.", detailOrder.estimatedDelivery ? new Date(detailOrder.estimatedDelivery).toLocaleDateString("es") : "—"],
                  ...
                  <div key={l} className="bg-white/5 rounded-xl px-3 py-2 flex justify-between items-center gap-2">
                    ...
                    {v === null ? <StatusBadge status={detailOrder.status} /> : <span className="text-white text-xs font-medium text-right">{v}</span>}
                  </div>
                ))}
              </div>
              {detailOrder.notes && (
              ...
              {NEXT_STATUSES[normalizeStatus(detailOrder.status)]?.length && (
                <button onClick={() => openStatusModal(detailOrder)} className="btn-primary w-full">
      ...
      <Modal open={createModal} onClose={() => setCreateModal(false)} title="Nuevo pedido" size="xl">
        <div className="grid grid-cols-2 gap-4 mb-4">
          ...
          <div>
            ...
            <input type="number" className="input" value={form.taxRate} onChange={e => setForm(f => ({ ...f, taxRate: Number(e.target.value) }))} />
          </div>
          <div>
            <label className="label">Fecha estimada de entrega</label>
            <input type="date" className="input" value={form.estimatedDelivery} onChange={e => setForm(f => ({ ...f, estimatedDelivery: e.target.value }))} />
          </div>
          <div className="col-span-2">
            <label className="label">Dirección de envío</label>
            <input className="input" value={form.shippingAddress} onChange={e => setForm(f => ({ ...f, shippingAddress: e.target.value }))} />
          </div>
        </div>
        <div className="mb-4">
          <div className="flex items-center justify-between mb-2">
            <p className="text-sm font-semibold text-slate-300">Líneas de producto</p>
            <button onClick={addItem} className="btn-secondary text-xs px-2 py-1"><Plus className="w-3 h-3" /> Agregar</button>
          </div>
          {form.items.length === 0 ? (
              ...
              {form.items.map((item, idx) => (
                <div key={idx} className="flex gap-2 items-end">
                  <div className="flex-1">
                    ...
                    <select className="input" value={item.productId} onChange={e => updateItem(idx, "productId", Number(e.target.value))}>
                  ...
                  <button onClick={() => removeItem(idx)} className="btn-danger px-2 py-2.5 text-xs mb-0.5"><XCircle className="w-4 h-4" /></button>
                </div>
              ))}
            </div>
          )}
      ...
      {/* ── Status Update Modal ────────────────────────────────────────────── */}
      <Modal open={!!statusModal} onClose={() => setStatusModal(null)} title={`Actualizar — ${statusModal?.orderNumber ?? ""}`} size="sm">
        {statusModal && (
